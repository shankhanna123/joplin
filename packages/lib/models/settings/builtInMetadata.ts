...
const builtInMetadata = (Setting: typeof SettingType) => {
	...
	const output = {
		...
		'inking.enabled': {
			...
			public: true,
			appTypes: [AppType.Desktop, AppType.Mobile],
			label: () => _('Enable inking and drawing features'),
			description: () => _('Allows drawing with stylus or finger on notes with handwriting recognition'),
			storage: SettingStorage.File,
			isGlobal: true,
		},
		'inking.autoStraighten': {
			value: true,
			type: SettingItemType.Bool,
			section: 'note', 
			public: true,
			appTypes: [AppType.Desktop, AppType.Mobile],
			label: () => _('Auto-straighten crooked lines'),
			description: () => _('Automatically converts roughly drawn lines to perfect straight lines'),
			storage: SettingStorage.File,
			isGlobal: true,
		},
		'inking.defaultBrushColor': {
			value: '#000000',
			type: SettingItemType.String,
			section: 'note',
			public: true,
			appTypes: [AppType.Desktop, AppType.Mobile],
			label: () => _('Default brush color'),
			description: () => _('Default color for drawing strokes'),
			storage: SettingStorage.File,
			isGlobal: true,
		},
		'inking.defaultBrushWidth': {
			value: 3,
			type: SettingItemType.Int,
			section: 'note',
			public: true,
			appTypes: [AppType.Desktop, AppType.Mobile],
			label: () => _('Default brush width'),
			description: () => _('Default width for drawing strokes in pixels'),
			minimum: 1,
			maximum: 50,
			storage: SettingStorage.File,
			isGlobal: true,
		},
		'inking.defaultBackgroundType': {
			value: 'none',
			type: SettingItemType.String,
			section: 'note',
			public: true,
			appTypes: [AppType.Desktop, AppType.Mobile],
			label: () => _('Default background type'),
			description: () => _('Default background pattern for drawing canvas'),
			isEnum: true,
			options: () => {
				return {
					'none': _('None'),
		...
		'inking.handwritingRecognition': {
			value: true,
			type: SettingItemType.Bool,
			section: 'note',
			public: true,
			appTypes: [AppType.Desktop, AppType.Mobile],
			label: () => _('Enable handwriting recognition'),
			description: () => _('Convert handwritten text to digital text using Google Vision API'),
			storage: SettingStorage.File,
			isGlobal: true,
		},
		'inking.infiniteCanvas': {
			value: true,
			type: SettingItemType.Bool,
			section: 'note',
			public: true,
			appTypes: [AppType.Desktop, AppType.Mobile],
			label: () => _('Enable infinite canvas'),
			description: () => _('Allow unlimited scrolling and zooming on drawing canvas'),
			storage: SettingStorage.File,
			isGlobal: true,
		},
